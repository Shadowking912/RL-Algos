...
def expectedsarsa(step_size,Q,epsilon):
    ...
    while True:
        ...
        Q[observation,action]=Q[observation,action]+step_size*(rew+discounting_factor*expectation(Q,new_state,epsilon)-Q[observation,action])
        # Q[observation,action]=Q[observation,action]+step_size*(rew+discounting_factor*Q[new_state,new_action]-Q[observation,action])
...
for j in tqdm(range(num_runs),total=num_runs):
    ...
    for i in range(num_episodes):
        cur_sum=expectedsarsa(step_size,Q,0.1)
        sums[i]+=cur_sum
sums=sums/num_runs
